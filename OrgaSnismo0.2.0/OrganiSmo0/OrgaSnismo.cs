	...
	public class OrgaSnismo:OrgaSnismoGui//,IDisposable
	{
				...
				if (!pasovacio) {
					if (ciclo < 1000)
						...
						if (!Anglo)
							...
							lbRegresivo.Text = "Next cicle: (seconds): " + segundos.ToString ();
					} else {
						lbRegresivo.Visible = true;
						...
						if (!Anglo)
							...
							lbRegresivo.Text = "Next cicle: (minutes): " + minutos.ToString ();
					}
		...
		internal override void Guardarimagen_Activated (object sender, EventArgs e)
		{
			...
			fichero = directorio + "/Imagenes/imagorg" + lbPoblacion.Text + " Ciclos: " + ciclos.ToString () + ".png";
				...
				fichero = directorio + "/Imagenes/imagorg" + lbPoblacion.Text + " Cicles: " + ciclos.ToString () + ".png";	
		...
		public override void Ayuda_Activated (object sender, EventArgs e)
		{
			...
			if (!Anglo) {
				...
				buffer.Text += "\nWhen the program starts, apart of a menu bar wich is commented later, the button 'Draw initial shape' appears.\nAfter do click on this button, the following elements are shown:\n\n" +
				"\tGrid: A new yellow cell is created by clicking over a square. Clicking over a previously created cell, erase it\n\n" +  
					"\t'manual cicle' check button: It is chequed by defect, and is used to stablish the cicle time by the user.\n\t\t\t\t\t\t\t When is unchequed, the check button and the spinbutton change theirs colors and \n\t\t\t\t\t\t\t the cicle time is stablished automaticaly function the time taken by the CPU to\n\t\t\t\t\t\t\t calculate the next shape, i.e. as fast as the CPU can process the algorithm." +
					"\n\n\tSpin button:  Shows the stablished cicle time. It is set at 100 milliseconds by defect.\n\t\t\t\tThe cicle time can be changed when 'manual cicle' is checked.\n\t\t\t\tWhen the time that the CPU take to calculate a new shape is bigger than the cicle time \n\t\t\t\tsettled down, the program change to auto by itself." +
					"\n\n\t'Accept' button: To start the cicles once the desired shaped has been drawn or loaded and the cicle time stablished." +
					"\n\nOnce the game is started, the drawn shape is going changing according to the rules at every new cicle.\nBy defect, the rules are the ones stablished by Conway. i.e.:\nA cell remain alive if it is adyacent to two alive cells, otherwise, the cell dies at next cicle.\nA cell borns, if it is adyacent to three alive cells." +
					"\n\nAt the window bottom left, the population ( alive cells acount ) and the cicles passed is shown." +
				...
				buffer.Text += "\n\nFile:" +
					...
					"\n\n\tSave: Save the current shape, rules and colors in a *.fm file\n\t\t   One can save a shape, either that the cicles have started or not. If the cicles are running, it is convenient \n\t\t   to do the file saving process with the game paused." +
					"\n\n\tLoad: Load a shape whith the rules and color previusly saved in a *.fm file." +
					"\n\n\tSave image: Save a *.png image of the current screen.\n\t\t\t      The png file, is saved in a directory called 'imagenes' which is created , if it dosen't exist, into \n\t\t\t      the directory where the application executable is." +
					"\n\nOptions:" +
					...
					"\n\n\tChange colors: The 5 buttons shown, control respectively the following colors:\n\t\t\t\t  The perimeter cell line ( red by defect ),\n\t\t\t\t  the alive cell interior ( yellow by defect ),\n\t\t\t\t  the new cells born interior ( yellow by defect ) \n\t\t\t\t  the died cells interior ( black by defect ) and \n\t\t\t\t  the background ( black by defect )." +
					"\n\n\tBack to values by defect: Return to the rules and colors by defect ." +
					"\n\n\tAutosave images: Save a *.png image of the new shape each cicle.\n\t\t\t\t       The png images are saved at the directory called 'imagenes', wich is created if it dosen't \n\t\t\t\t       exist, into the directory where the application executable is.\n\t\t\t\t       The file name of each png saved image, contains the population and cicles information." +
